...
- [2. 注册账号](#2-注册账号)
- [3. 部署](#3-部署)
    - [3.1. ~~方式一：Actions自动部署~~（暂时不再支持）](#31-方式一actions自动部署暂时不再支持)
...
## 3. 部署
以下两种方式任选一个适合自己需求的即可。
### 3.1. ~~方式一：Actions自动部署~~（暂时不再支持）
因为代码仓库没了，所以该方式暂时不再支持，请跳到下一节。
...
#### 3.2.1. 下载压缩包到本地
点击[BiliBiliTool/release](https://github.com/RayWangQvQ/BiliBiliTool.Docs/releases)，选择最新版本的 `tencent-scf.zip` ，下载到本地
#### 3.2.2. 云函数控制台新增函数服务
